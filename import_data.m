...
for R1 = 1:names_total_num
    for letter = 1:letters_num
        if letter == 1
            temp = "a";
            A_Vt{letter, R1} = strcat(num2str(R1), temp, "-Vt.data");
            A_It{letter, R1} = strcat(num2str(R1), temp, "-It.data");
        elseif letter == 2
            temp = "b";
            A_Vt{letter, R1} = strcat(num2str(R1), temp, "-Vt.data");
            A_It{letter, R1} = strcat(num2str(R1), temp, "-It.data");
...
data_Vt = cell(names_total_num, letter);
data_It = cell(names_total_num, letter);
